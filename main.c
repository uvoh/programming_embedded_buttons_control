...
/* USER CODE END PTD nnnnnnnnnnnnnnnnnnnn*/

/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
typedef enum { NOBUTTON = 0, BUTTON1 = 1, BUTTON2, BUTTON3, BUTTON4, BUTTON5, BUTTON6, BUTTON7,
BUTTON8} ButtonNumber;
...
//Add the LCD screen variables:
#define FONT_WIDTH 16
#define FONT_HEIGHT 24
#define NUM_BTN_HEIGHT 60
#define NUM_BTN_WIDTH 60
...
void SetLEDcolor(LEDnumber led, LEDcolour Colour) {
	...
	for (int i = 0; i < 16; i++) {
		...
		/* Do a clock pulse */
		HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9, GPIO_PIN_RESET); // CLK_PIN
		HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9, GPIO_PIN_SET); // CLK_PIN
	}
	HAL_Delay(1);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  ButtonNumber currentButton;
  unsigned short LEDPos;
  /* USER CODE END Init */
...
//  BSP_LCD_DisplayStringAt(0, FONT_HEIGHT * 3, (uint8_t *)"CE865", CENTER_MODE);
//  BSP_LCD_DisplayStringAt(0, FONT_HEIGHT * 4, (uint8_t *)"ASSIGNMENT 1", CENTER_MODE);
//  BSP_LCD_DisplayStringAt(0, FONT_HEIGHT * 6, (uint8_t *)"1234567", CENTER_MODE); //your ID number
  HAL_Delay(2000);
  /* USER CODE END 2 */
  ...
  int BLINKINGBUTTON=1;
  while (1)
  {

	  //flashing of the LED1 to Signify the start of the test
	  LEDPos=BLINKINGBUTTON-1;
	  SetLEDcolor(LEDPos,RED);
	  HAL_Delay(200);
	  SetLEDcolor(LEDPos,GREEN);
	  HAL_Delay(200);


	  currentButton = GetButtonPressed();
	  while (currentButton== BLINKINGBUTTON)
	  {
		  s = 0;
		  m = 0;
 		  char buff[12];
 		  sprintf(buff, "%d.%03d", s,m);
 		  BSP_LCD_DisplayStringAt(0, FONT_HEIGHT * 6, (uint8_t *)buff, CENTER_MODE); //your ID number
		  ...
		  int randombutton = rand() %8;
		 	  while(1){
		 		 ...
		 		 currentButton = GetButtonPressed();
		 		 if (currentButton-1 == randombutton){
		 			 break;
		 		 }
		 	  }break;
	  }
